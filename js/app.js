(() => {
    var __webpack_modules__ = {
        11: module => {
            ...
            function prefix(key) {
                ...
                if (void 0 !== style[key]) return key;
    ...
    (() => {
        ...
        function initSliders() {
            ...
            if (document.querySelector(".inprocess__slider")) new core(".inprocess__slider", {
                modules: [ Navigation ],
                observer: true,
                observeParents: true,
                slidesPerView: 2.5,
                ...
                loop: true,
                navigation: {
                    prevEl: ".inprocess__slider .swiper-button-prev",
                    nextEl: ".inprocess__slider .swiper-button-next"
                },
                breakpoints: {
                    320: {
                        slidesPerView: 1.1,
            ...
            if (document.querySelector(".partners__slider")) new core(".partners__slider", {
                ...
                navigation: {
                    prevEl: ".page__partners .swiper-button-prev",
                    nextEl: ".page__partners .swiper-button-next"
                },
                breakpoints: {
                    320: {
                        slidesPerView: 2,
        ...
        var prefix_default = __webpack_require__.n(prefix);
        function mapEach(element, callback) {
            if (element instanceof window.HTMLElement) return [ callback(element) ];
            if ("object" === typeof element) return Object.keys(element).map((key => callback(element[key]))); else return element.map(callback);
        }
        const cssEasing = {
        ...
        const getEasing = easing => {
            if ("undefined" === easing) return cssEasing.default;
            if (easing && !cssEasing[easing]) throw new Error(`Invalid Easing ${easing} option.`);
        ...
        function split({element, expression = " ", append = true}) {
            ...
            words.forEach((word => {
                ...
                } else innerHTML += parseLine(word);
            }));
            element.innerHTML = innerHTML;
            const spans = element.querySelectorAll("span");
            if (append) spans.forEach((span => {
                const isSingleLetter = 1 === span.textContent.length;
        ...
        function parseLine(line) {
            if ("" === line) return line; else if (" " === line) return "&nbsp;"; else {
                line = line.trim();
                return "<br>" === line ? "<br>" : `<span>${line}</span>` + (line.length > 1 ? " " : "");
            }
        }
        const DEFAULT = {
            duration: 1450,
        ...
        const isBrowser = "undefined" !== typeof window && "undefined" !== typeof document && "undefined" !== typeof navigator && "undefined" !== typeof requestAnimationFrame;
        class Animation {
            ...
            createObserver() {
                if (isBrowser) this.observer = new window.IntersectionObserver(((entries, observer) => {
        ...
        class Text extends Animation {
            constructor({element, options = {}}) {
                ...
                if (0 !== paragraphs.length) paragraphs.forEach((element => {
                    split({
                        element
                    });
                    split({
            ...
            addStyles(word, index) {
                word.style.transition = `transform ${this.options.duration}ms ${index * this.options.stagger}ms ${this.options.easing}, opacity ${this.options.fadeDuration}ms ${index * this.options.stagger}ms ${this.options.fadeEasing}`;
            ...
            animateIn() {
                super.animateIn();
                if ("undefined" === typeof this.lines) return;
                if ("object" === typeof this.lines) Object.keys(this.lines).forEach(((key, index) => {
                    this.lines[key].forEach((word => {
                        this.addStyles(word, index);
                    }));
                })); else this.lines.forEach(((line, index) => {
                    line.forEach((word => {
                        this.addStyles(word, index);
                    }));
                }));
            ...
            animateOut() {
                super.animateOut();
                if ("undefined" === typeof this.lines) return;
                if ("object" === typeof this.lines) Object.keys(this.lines).forEach((key => {
                    this.lines[key].forEach((word => {
                        word.style[this.transformPrefix] = `translateY(${this.options.reveal ? `${this.options.top ? "-" : ""}150%` : "0%"})  scale(${this.options.scale}) rotate(${this.options.rotation}deg)`;
                        this.options.fade ? word.style.opacity = "0" : null;
                    }));
                })); else this.lines.forEach((line => {
                    line.forEach((word => {
                        word.style[this.transformPrefix] = `translateY(${this.options.reveal ? `${this.options.top ? "-" : ""}150%` : "0%"})  scale(${this.options.scale}) rotate(${this.options.rotation}deg)`;
                        this.options.fade ? word.style.opacity = "0" : null;
                    }));
        ...
        class Title extends Animation {
            constructor({element, options = {}}) {
                ...
                if (titles.includes(element.tagName)) {
                    ...
                    if (0 !== texts.length) texts.forEach((element => {
                        split({
                            element
                        });
                        const newWords = isBrowser ? element.querySelectorAll("span") : [];
                ...
                } else throw new Error("Textify.js: The target element must be a heading tag.");
                super({
                    element,
                    elements: {
                        words,
                ...
                if (!titles.includes(element.tagName)) return;
            ...
            animateIn() {
                ...
                this.elements.chars.forEach(((char, index) => {
                    char.style.transition = `transform ${this.options.duration}ms ${index * this.options.stagger}ms ${this.options.easing}, opacity ${this.options.fadeDuration}ms ${index * this.options.stagger}ms ${this.options.fadeEasing}`;
            ...
            animateOut() {
                ...
                this.elements.chars.forEach((char => {
                    char.style[this.transformPrefix] = `translateY(${this.options.reveal ? `${this.options.top ? "-" : ""}150%` : "0%"}) rotate(${this.options.rotation}deg) scale(${this.options.scale})`;
        ...
        class TextifyTitle {
            constructor(options = {}) {
                if (!options.easing) options.easing = getEasing("default"); else try {
                    options.easing = getEasing(options.easing);
                } catch (err) {
                    throw new Error(err);
                }
                if (!options.fadeEasing) options.fadeEasing = getEasing("default"); else try {
                ...
                if (isBrowser) {
                    ...
                    this.animations = mapEach(this.elements, (element => new Title({
                        element,
                        options: controller
                    })));
                    this.events();
        ...
        const plugins_TextifyTitle = TextifyTitle;
        class Textify {
            constructor(options = {}) {
                if (!options.easing) options.easing = getEasing("default"); else try {
                    options.easing = getEasing(options.easing);
                ...
                if (isBrowser) {
                    if (!document.querySelector(DEFAULT_TARGET_ELEMENT_SELECTOR)) throw new Error("No element found with selector: " + DEFAULT_TARGET_ELEMENT_SELECTOR);
                    this.elements = document.querySelectorAll(DEFAULT_TARGET_ELEMENT_SELECTOR);
                    this.animations = mapEach(this.elements, (element => new Text({
                        element,
                        options: controller
                    })));
        ...
        const src = {
            Textify,
            TextifyTitle: plugins_TextifyTitle
        };
        if (isBrowser) {
            window.Textify = Textify;
            window.TextifyTitle = plugins_TextifyTitle;
        }
        const {Textify: textify_Textify} = src;
        new textify_Textify({
            selector: ".title",
            duration: 1e3,
            stagger: 100,
        ...
        da.init();
        const inputAdd = document.querySelector(".form__add");
        inputAdd.addEventListener("change", (function(e) {
            console.log(inputAdd.files[0].name);
            const formRow = inputAdd.closest(".form__row");
            formRow.querySelector(".form__add-text").innerHTML = inputAdd.files[0].name;
            formRow.classList.add("add");
        }));
        const mouse = document.querySelector(".mouse");
        if (mouse) {
            function moveMouse(e) {
                if (e.target.closest(".content-advantages__body")) {
                    mouse.classList.remove("mouse-hide");
                    mouse.classList.add("mouse-active");
                    const currentRow = e.target.closest(".content-advantages__row");
                    const currentRowId = currentRow.dataset.row;
                    const mouseWrapper = document.querySelector(".mouse__wrapper");
                    const mouseImages = Array.from(mouseWrapper.querySelectorAll(".mouse__image"));
                    const currentImage = mouseImages[currentRowId - 1];
                    mouseImages.forEach((mouseImage => {
                        mouseImage.classList.contains("active") ? mouseImage.classList.remove("active") : null;
                    }));
                    currentImage.classList.add("active");
                    mouse.style.transform = `translate(-50%, -50%) translate(${e.clientX - 0}px, ${e.clientY - 0}px)`;
                } else {
                    mouse.classList.contains("mouse-active") ? mouse.classList.remove("mouse-active") : null;
                    mouse.classList.add("mouse-hide");
                    document.querySelector(".mouse__image.active") ? document.querySelector(".mouse__image.active").classList.remove("active") : null;
                }
